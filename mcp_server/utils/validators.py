...
Provides unified parameter validation functions.
"""
...
def get_supported_platforms() -> List[str]:
    """
    Dynamically get the list of supported platforms from config.yaml

    Returns:
        List of platform IDs

    Note:
        - Returns empty list on read failure, allowing all platforms to pass (fallback strategy)
...
def validate_platforms(platforms: Optional[List[str]]) -> List[str]:
    ...
    Note:
        ...
        - Validates whether platform IDs are in the platforms configuration of config.yaml
    ...
    # If config loading fails (supported_platforms is empty), allow all platforms to pass
    if not supported_platforms:
        print("Warning: Platform configuration not loaded, skipping platform validation")
        return platforms

    # Verify each platform is in configuration
...
def validate_date_range(date_range: Optional[dict]) -> Optional[tuple]:
    ...
    if start_date.date() > today or end_date.date() > today:
        ...
        raise InvalidParameterError(
            f"Future date queries not allowed: {', '.join(future_dates)} (Current date: {today.strftime('%Y-%m-%d')})",
...
def validate_keyword(keyword: str) -> str:
    ...
    if not keyword:
        raise InvalidParameterError("keyword cannot be blank characters")

    if len(keyword) > 100:
        raise InvalidParameterError(
            "keyword length cannot exceed 100 characters",
            suggestion="Please use more concise keywords"
        )
...
def validate_date_query(
    ...
    if not date_query:
        raise InvalidParameterError(
            ...
            suggestion="Please provide date query, such as: today, yesterday, 2025-10-10"
        )
